...
const pathOptions = computed(() => {
  ...
  const fixed = [
    { label: '首页（/dashboard）', value: '/dashboard' },
    { label: '我的缺陷（/bug/my）', value: '/bug/my' },
    { label: '缺陷看板（/bug/statistics）', value: '/bug/statistics' },
  ]
  ...
  <div class="space-y-4 p-4 sm:p-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="text-muted-foreground">配置不同角色登录首页、首页替代路径和默认展开菜单。</p>
      </div>
    ...
    <Dialog v-model:open="dialogOpen">
      <DialogContent class="sm:max-w-2xl">
        ...
        <div class="grid gap-4 py-2 md:grid-cols-2">
          ...
          <div class="grid gap-2"><Label>首页替代路径</Label><Select v-model="form.dashboardPath"><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem v-for="item in pathOptions" :key="item.value" :value="item.value">{{ item.label }}</SelectItem></SelectContent></Select></div>
