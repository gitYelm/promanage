...
import { useMilestoneColumns } from './milestone-columns'

const { toast } = useToast()
const { columns, visibleColumnMap, toggleColumn, resetColumns } = useMilestoneColumns()
...
const query = reactive({ pageNum: 1, pageSize: 20, keyword: '', projectId: PM_ALL_OPTION_VALUE, status: PM_ALL_OPTION_VALUE, stage: PM_ALL_OPTION_VALUE, ownerId: PM_ALL_OPTION_VALUE, targetDateStart: '', targetDateEnd: '', sortBy: '', sortOrder: '' as 'asc' | 'desc' | '' })
...
async function getList() {
  loading.value = true
  try {
    const res = await listMilestones({ ...query, projectId: pmNormalizeAll(query.projectId), status: pmNormalizeAll(query.status), stage: pmNormalizeAll(query.stage), ownerId: pmNormalizeAll(query.ownerId), targetDateStart: toLocalDateBoundaryIso(query.targetDateStart, 'start'), targetDateEnd: toLocalDateBoundaryIso(query.targetDateEnd, 'end') })
    rows.value = res.rows; total.value = res.total
  } finally { loading.value = false }
}
function searchList() { query.pageNum = 1; getList() }
function resetQuery() { Object.assign(query, { pageNum: 1, pageSize: query.pageSize, keyword: '', projectId: PM_ALL_OPTION_VALUE, status: PM_ALL_OPTION_VALUE, stage: PM_ALL_OPTION_VALUE, ownerId: PM_ALL_OPTION_VALUE, targetDateStart: '', targetDateEnd: '', sortBy: '', sortOrder: '' }); getList() }
...
<template>
  <div class="space-y-4 p-4 sm:p-6">
    <div class="flex items-center justify-between"><div><h2 class="text-2xl font-bold">里程碑</h2><p class="text-sm text-muted-foreground">管理开发完成、内测开始、发布上线等关键节点。</p></div><div class="flex gap-2"><Button v-hasPermi="['pm:milestone:manage']" @click="add">新增里程碑</Button><TableColumnSettingsButton :columns="columns" @toggle="toggleColumn" @reset="resetColumns" /><TableRefreshIconButton :loading="loading" @refresh="getList" /></div></div>
    <TableFilterPanel description="默认展示里程碑名称、项目和状态，展开后可按阶段、负责人和目标日期完整筛选。">
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div class="space-y-1"><label for="milestone-filter-keyword" class="text-sm font-medium">里程碑名称</label><Input id="milestone-filter-keyword" v-model="query.keyword" placeholder="里程碑名称" @keyup.enter="searchList" /></div>
        <div class="space-y-1"><label for="milestone-filter-project" class="text-sm font-medium">所属项目</label><Select v-model="query.projectId"><SelectTrigger id="milestone-filter-project"><ProjectSelectValue :model-value="query.projectId" :projects="projects" :all-value="PM_ALL_OPTION_VALUE" /></SelectTrigger><SelectContent><SelectItem :value="PM_ALL_OPTION_VALUE">全部项目</SelectItem><SelectItem v-for="p in projects" :key="p.projectId" :value="p.projectId"><ProjectSelectOption :name="p.projectName" :code="p.projectKey" :stage="p.projectStage" :owner-name="p.owner?.nickName || p.owner?.userName" /></SelectItem></SelectContent></Select></div>
        <div class="space-y-1"><label for="milestone-filter-status" class="text-sm font-medium">状态</label><Select v-model="query.status"><SelectTrigger id="milestone-filter-status"><SelectValue /></SelectTrigger><SelectContent><SelectItem :value="PM_ALL_OPTION_VALUE">全部状态</SelectItem><SelectItem v-for="s in PM_MILESTONE_STATUS_OPTIONS" :key="s.value" :value="s.value">{{ s.label }}</SelectItem></SelectContent></Select></div>
      ...
      <template #expanded>
        <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div class="space-y-1"><label for="milestone-filter-stage" class="text-sm font-medium">对应阶段</label><Select v-model="query.stage"><SelectTrigger id="milestone-filter-stage"><SelectValue /></SelectTrigger><SelectContent><SelectItem :value="PM_ALL_OPTION_VALUE">全部阶段</SelectItem><SelectItem v-for="s in PM_PROJECT_STAGE_OPTIONS" :key="s.value" :value="s.value">{{ s.label }}</SelectItem></SelectContent></Select></div>
          <div class="space-y-1"><label for="milestone-filter-owner" class="text-sm font-medium">负责人</label><Select v-model="query.ownerId"><SelectTrigger id="milestone-filter-owner"><SelectValue /></SelectTrigger><SelectContent><SelectItem :value="PM_ALL_OPTION_VALUE">全部负责人</SelectItem><SelectItem v-for="u in users" :key="u.userId" :value="u.userId">{{ u.nickName || u.userName }}</SelectItem></SelectContent></Select></div>
          <FilterRangeField v-model:start="query.targetDateStart" v-model:end="query.targetDateEnd" label="目标日期" />
        </div>
      </template>
    </TableFilterPanel>
    <div class="rounded-md border"><Table><TableHeader><TableRow><SortableTableHead v-if="visibleColumnMap.milestoneName" label="里程碑" sort-key="milestoneName" :sort-by="query.sortBy" :sort-order="query.sortOrder" @sort="handleSort" /><SortableTableHead v-if="visibleColumnMap.project" label="项目" sort-key="projectId" :sort-by="query.sortBy" :sort-order="query.sortOrder" @sort="handleSort" /><SortableTableHead v-if="visibleColumnMap.stage" label="阶段" sort-key="stage" align="center" :sort-by="query.sortBy" :sort-order="query.sortOrder" @sort="handleSort" /><SortableTableHead v-if="visibleColumnMap.owner" label="负责人" sort-key="ownerId" :sort-by="query.sortBy" :sort-order="query.sortOrder" @sort="handleSort" /><SortableTableHead v-if="visibleColumnMap.targetDate" label="目标日期" sort-key="targetDate" :sort-by="query.sortBy" :sort-order="query.sortOrder" @sort="handleSort" /><SortableTableHead v-if="visibleColumnMap.status" label="状态" sort-key="status" align="center" :sort-by="query.sortBy" :sort-order="query.sortOrder" @sort="handleSort" /><TableHead v-if="canShowQuickActionColumn && visibleColumnMap.quickActions" class="min-w-48 text-left">快捷操作</TableHead><TableHead v-if="canShowOperationColumn && visibleColumnMap.actions" class="w-36 text-right">操作</TableHead></TableRow></TableHeader><TableBody><TableRow v-for="row in rows" :key="row.milestoneId"><TableCell v-if="visibleColumnMap.milestoneName"><div class="font-medium">{{ row.milestoneName }}</div><div class="text-xs text-muted-foreground">{{ row.completionCriteria || '-' }}</div></TableCell><TableCell v-if="visibleColumnMap.project"><ProjectBadge :name="row.project?.projectName" :code="row.project?.projectKey" /></TableCell><TableCell v-if="visibleColumnMap.stage" class="text-center"><StatusBadge domain="projectStage" :value="row.stage" /></TableCell><TableCell v-if="visibleColumnMap.owner">{{ row.owner?.nickName || '-' }}</TableCell><TableCell v-if="visibleColumnMap.targetDate">{{ formatDate(row.targetDate) }}</TableCell><TableCell v-if="visibleColumnMap.status" class="text-center"><StatusBadge domain="milestone" :value="row.status" /></TableCell><TableCell v-if="canShowQuickActionColumn && visibleColumnMap.quickActions"><div v-if="quickActions(row).length" class="flex flex-wrap gap-2"><SemanticActionButton v-for="item in quickActions(row)" :key="item.action" :permissions="item.permissions" :action="item.action" @click="action(row, item.action)">{{ item.label }}</SemanticActionButton></div><span v-else class="text-sm text-muted-foreground">-</span></TableCell><TableCell v-if="canShowOperationColumn && visibleColumnMap.actions" class="text-right"><div class="flex justify-end gap-2"><Button v-hasPermi="['pm:milestone:manage']" size="sm" variant="outline" @click="edit(row)">编辑</Button><Button v-hasPermi="['pm:milestone:manage']" size="sm" variant="destructive" @click="remove(row)">删除</Button></div></TableCell></TableRow></TableBody></Table><EmptyState v-if="!rows.length" /></div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <div class="space-y-2">
            ...
            <Select v-model="form.projectId" @update:model-value="(v) => { form.projectId = String(v); form.ownerId = PM_NONE_OPTION_VALUE; refreshAssignableUsers(form.projectId) }">
              <SelectTrigger id="milestone-project"><ProjectSelectValue :model-value="form.projectId" :projects="projects" placeholder="请选择所属项目" /></SelectTrigger>
