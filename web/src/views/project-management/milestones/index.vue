...
function quickActions(row: Milestone) { return pmAvailableActions(PM_MILESTONE_ACTION_OPTIONS, row.status) }
...
<template>
  <div class="space-y-4 p-4 sm:p-6">
    ...
    <div class="rounded-md border"><Table><TableHeader><TableRow><TableHead>里程碑</TableHead><TableHead>项目</TableHead><TableHead class="text-center">阶段</TableHead><TableHead>负责人</TableHead><TableHead>目标日期</TableHead><TableHead class="text-center">状态</TableHead><TableHead class="min-w-48 text-left">快捷操作</TableHead><TableHead class="w-36 text-right">操作</TableHead></TableRow></TableHeader><TableBody><TableRow v-for="row in rows" :key="row.milestoneId"><TableCell><div class="font-medium">{{ row.milestoneName }}</div><div class="text-xs text-muted-foreground">{{ row.completionCriteria || '-' }}</div></TableCell><TableCell>{{ row.project?.projectName }}</TableCell><TableCell class="text-center"><StatusBadge domain="projectStage" :value="row.stage" /></TableCell><TableCell>{{ row.owner?.nickName || '-' }}</TableCell><TableCell>{{ formatDate(row.targetDate) }}</TableCell><TableCell class="text-center"><StatusBadge domain="milestone" :value="row.status" /></TableCell><TableCell><div v-if="quickActions(row).length" class="flex flex-wrap gap-2"><SemanticActionButton v-for="item in quickActions(row)" :key="item.action" v-hasPermi="['pm:milestone:manage']" :action="item.action" @click="action(row, item.action)">{{ item.label }}</SemanticActionButton></div><span v-else class="text-sm text-muted-foreground">-</span></TableCell><TableCell class="text-right"><div class="flex justify-end gap-2"><Button v-hasPermi="['pm:milestone:manage']" size="sm" variant="outline" @click="edit(row)">编辑</Button><Button v-hasPermi="['pm:milestone:manage']" size="sm" variant="destructive" @click="remove(row)">删除</Button></div></TableCell></TableRow></TableBody></Table><EmptyState v-if="!rows.length" /></div>
