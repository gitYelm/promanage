...
import { PM_PROJECT_STAGE_OPTIONS, PM_REQUIREMENT_STATUS_OPTIONS, PM_RISK_LEVEL_OPTIONS, formatDate, pmLabel, toDateInput } from '../shared/options'
...
const cards = computed<Array<{ label: string; value: string | number; tone: SemanticTone; description: string }>>(() => [
  ...
  { label: 'Bug 关闭率', value: `${overview.value?.bugCloseRate ?? 0}%`, tone: 'success', description: '已关闭 Bug 占比' },
  { label: '当前需求', value: overview.value?.counts.currentRequirements ?? 0, tone: 'info', description: '当前正在处理的需求' },
  { label: '修复中 Bug', value: overview.value?.counts.currentBugs ?? 0, tone: 'warning', description: '当前修复中的问题' },
...
<template>
  <div class="space-y-4 p-4 sm:p-6">
    ...
    <div v-if="overview" class="grid gap-4 lg:grid-cols-[2fr_1fr]">
      <Card><CardHeader><CardTitle>{{ overview.project.projectName }}</CardTitle></CardHeader><CardContent class="space-y-4"><div class="flex flex-wrap gap-2"><Badge>{{ pmLabel(PM_PROJECT_STAGE_OPTIONS, overview.project.projectStage) }}</Badge><RiskBadge :value="overview.project.riskLevel" /><Badge variant="outline">负责人：{{ overview.project.owner?.nickName || '-' }}</Badge><Badge variant="outline">计划：{{ formatDate(overview.project.plannedStartTime) }} - {{ formatDate(overview.project.plannedEndTime) }}</Badge></div><div class="flex items-center gap-3"><Progress :model-value="overview.progress" class="h-3" /><span class="w-12 text-sm font-medium">{{ overview.progress }}%</span></div><p class="text-sm text-muted-foreground">风险说明：{{ overview.project.riskNote || '暂无' }}</p></CardContent></Card>
      <Card><CardHeader><CardTitle>下一关键节点</CardTitle></CardHeader><CardContent><div v-if="overview.nextMilestone" class="space-y-2 text-sm"><div class="font-medium">{{ overview.nextMilestone.milestoneName }}</div><div>阶段：{{ pmLabel(PM_PROJECT_STAGE_OPTIONS, overview.nextMilestone.stage) }}</div><div>目标日期：{{ formatDate(overview.nextMilestone.targetDate) }}</div><div>负责人：{{ overview.nextMilestone.owner?.nickName || '-' }}</div></div><EmptyState v-else title="暂无下一节点" /></CardContent></Card>
    </div>
    ...
    <div v-if="overview" class="grid gap-4 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>当前处理需求</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>需求</TableHead><TableHead>负责人</TableHead><TableHead>状态</TableHead><TableHead>计划完成</TableHead></TableRow></TableHeader><TableBody><TableRow v-for="row in overview.currentRequirements" :key="row.requirementId"><TableCell>{{ row.title }}</TableCell><TableCell>{{ ownerName(row) }}</TableCell><TableCell>{{ pmLabel(PM_REQUIREMENT_STATUS_OPTIONS, row.status) }}</TableCell><TableCell>{{ formatDate(row.plannedEndTime) }}</TableCell></TableRow></TableBody></Table><EmptyState v-if="!overview.currentRequirements.length" /></CardContent></Card>
      <Card><CardHeader><CardTitle>当前修复 Bug</CardTitle></CardHeader><CardContent><div class="space-y-2"><div v-for="row in overview.currentBugs" :key="String(row.ticketId)" class="rounded-md border p-3 text-sm"><div class="font-medium">{{ bugTitle(row) }}</div><div class="text-muted-foreground">{{ String(row.status || '-') }} · {{ String(row.priority || '-') }}</div></div><EmptyState v-if="!overview.currentBugs.length" /></div></CardContent></Card>
      <Card><CardHeader><CardTitle>未处理需求</CardTitle></CardHeader><CardContent><div class="space-y-2"><div v-for="row in overview.pendingRequirements" :key="row.requirementId" class="rounded-md border p-3 text-sm"><div class="font-medium">{{ row.title }}</div><div class="text-muted-foreground">{{ pmLabel(PM_REQUIREMENT_STATUS_OPTIONS, row.status) }} · {{ ownerName(row) }}</div></div><EmptyState v-if="!overview.pendingRequirements.length" /></div></CardContent></Card>
      <Card><CardHeader><CardTitle>未处理 Bug</CardTitle></CardHeader><CardContent><div class="space-y-2"><div v-for="row in overview.pendingBugs" :key="String(row.ticketId)" class="rounded-md border p-3 text-sm"><div class="font-medium">{{ bugTitle(row) }}</div><div class="text-muted-foreground">{{ String(row.status || '-') }} · {{ String(row.priority || '-') }}</div></div><EmptyState v-if="!overview.pendingBugs.length" /></div></CardContent></Card>
      <Card><CardHeader><CardTitle>历史完成需求</CardTitle></CardHeader><CardContent><div class="space-y-2"><div v-for="row in overview.completedRequirements" :key="row.requirementId" class="rounded-md border p-3 text-sm"><div class="font-medium">{{ row.title }}</div><div class="text-muted-foreground">{{ pmLabel(PM_REQUIREMENT_STATUS_OPTIONS, row.status) }} · {{ formatDate(row.actualEndTime || row.updateTime) }}</div></div><EmptyState v-if="!overview.completedRequirements.length" /></div></CardContent></Card>
      <Card><CardHeader><CardTitle>历史修复 Bug</CardTitle></CardHeader><CardContent><div class="space-y-2"><div v-for="row in overview.completedBugs" :key="String(row.ticketId)" class="rounded-md border p-3 text-sm"><div class="font-medium">{{ bugTitle(row) }}</div><div class="text-muted-foreground">{{ String(row.status || '-') }} · {{ String(row.updateTime || '-') }}</div></div><EmptyState v-if="!overview.completedBugs.length" /></div></CardContent></Card>
