...
<template>
  <div class="space-y-4 p-4 sm:p-6">
    ...
    <Dialog v-model:open="progressOpen"><DialogContent class="max-w-2xl"><DialogHeader><DialogTitle>更新项目进度</DialogTitle><DialogDescription>更新阶段、进度、计划时间和风险信息；日期会用于看板统计和延期判断。</DialogDescription></DialogHeader><div class="grid gap-4 md:grid-cols-2"><FormFieldBlock label="项目阶段" field-id="pm-progress-stage" description="用于项目看板、老板驾驶舱和阶段统计。"><Select v-model="progressForm.projectStage"><SelectTrigger id="pm-progress-stage"><SelectValue /></SelectTrigger><SelectContent><SelectItem v-for="s in PM_PROJECT_STAGE_OPTIONS" :key="s.value" :value="s.value">{{ s.label }}</SelectItem></SelectContent></Select></FormFieldBlock><FormFieldBlock label="风险等级" field-id="pm-progress-risk" description="风险等级会以安全色展示，影响管理层关注优先级。"><Select v-model="progressForm.riskLevel"><SelectTrigger id="pm-progress-risk"><SelectValue /></SelectTrigger><SelectContent><SelectItem v-for="r in PM_RISK_LEVEL_OPTIONS" :key="r.value" :value="r.value">{{ r.label }}</SelectItem></SelectContent></Select></FormFieldBlock><FormFieldBlock label="计划开始" field-id="pm-progress-plan-start" optional description="用于排期统计；不确定时可先留空。"><Input id="pm-progress-plan-start" v-model="progressForm.plannedStartTime" type="date" /></FormFieldBlock><FormFieldBlock label="计划完成" field-id="pm-progress-plan-end" optional description="用于延期判断和交付计划统计；不确定时可先留空。"><Input id="pm-progress-plan-end" v-model="progressForm.plannedEndTime" type="date" /></FormFieldBlock><FormFieldBlock label="实际开始" field-id="pm-progress-actual-start" optional description="用于记录真实启动时间，便于复盘计划偏差。"><Input id="pm-progress-actual-start" v-model="progressForm.actualStartTime" type="date" /></FormFieldBlock><FormFieldBlock label="实际完成" field-id="pm-progress-actual-end" optional description="用于计算实际交付周期和完成统计。"><Input id="pm-progress-actual-end" v-model="progressForm.actualEndTime" type="date" /></FormFieldBlock><FormFieldBlock :label="`项目进度：${progressForm.progress ?? 0}%`" field-id="pm-progress-percent" class="md:col-span-2" description="填写 0 到 100 的百分比，代表当前项目综合推进程度。"><Input id="pm-progress-percent" v-model="progressForm.progress" type="number" min="0" max="100" /></FormFieldBlock><FormFieldBlock label="风险说明" field-id="pm-progress-risk-note" optional class="md:col-span-2" description="记录延期、质量、资源或依赖风险，便于管理层判断风险原因。"><Textarea id="pm-progress-risk-note" v-model="progressForm.riskNote" /></FormFieldBlock><FormFieldBlock label="操作说明" field-id="pm-progress-remark" optional class="md:col-span-2" description="说明本次阶段、进度或风险调整原因，便于历史追溯。"><Textarea id="pm-progress-remark" v-model="progressForm.remark" placeholder="说明本次阶段/进度/风险调整原因" /></FormFieldBlock></div><DialogFooter><Button @click="saveProgress">保存</Button></DialogFooter></DialogContent></Dialog>
  </div>
</template>
