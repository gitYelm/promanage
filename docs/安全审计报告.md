...
| 传输安全 | ⭐⭐⭐⭐⭐ | CORS 白名单 + Helmet 安全头 + 生产环境强制配置 |
...
### 2.7 安全响应头 (Helmet)
- ✅ Content-Security-Policy (CSP) - 限制页面可加载的资源来源
- ✅ X-Frame-Options - 防止页面被嵌入 iframe，阻止点击劫持
...
### 2.8 请求限流 (Rate Limiting)
- ✅ 全局请求频率限制（每 IP 每分钟 100 次）
...
#### ✅ L3: Rate Limiting (已修复)
**原问题**: 未发现全局请求频率限制，可能遭受 DoS 攻击。

**修复方案**: 已添加 `@nestjs/throttler` 全局限流模块
- 每个 IP 每分钟最多 100 次请求
- 超出限制返回 429 Too Many Requests
...
| 错误处理 | ✅ | 全局异常过滤器 |
| CORS 配置 | ✅ | 生产环境白名单模式 |
| HTTPS | ⚠️ | 需在反向代理 (Nginx/Caddy) 配置证书 |
| CSP 头 | ✅ | Helmet 已配置 |
| Rate Limiting | ✅ | @nestjs/throttler 已配置 |
...
│       │                                                         │
│       ▼                                                         │
│  Helmet 安全头 (CSP, X-Frame-Options, HSTS...)                  │
│       │                                                         │
│       ▼                                                         │
