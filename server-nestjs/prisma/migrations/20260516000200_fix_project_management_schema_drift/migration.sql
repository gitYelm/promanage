-- Repair project-management schema drift for Bug feedback system.
--
-- Context:
-- `20260515000100_add_project_management` introduced project stage fields,
-- iterations, milestones, requirements and Bug links. Some existing databases can
-- have Prisma schema/client already updated while the physical PostgreSQL schema
-- is still missing part of those changes, causing Prisma P2022 errors such as:
--   column bug_project.project_stage does not exist
--   column bug_ticket.requirement_id does not exist
--
-- Keep this migration idempotent so it can safely repair partially migrated
-- development/test databases without depending on the exact drift point.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_iteration_project_id_fkey'
      AND conrelid = to_regclass('project_iteration')
  ) THEN
    ALTER TABLE "project_iteration"
      ADD CONSTRAINT "project_iteration_project_id_fkey"
      FOREIGN KEY ("project_id")
      REFERENCES "bug_project"("project_id")
      ON DELETE RESTRICT
      ON UPDATE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_iteration_owner_id_fkey'
      AND conrelid = to_regclass('project_iteration')
  ) THEN
    ALTER TABLE "project_iteration"
      ADD CONSTRAINT "project_iteration_owner_id_fkey"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_milestone_project_id_fkey'
      AND conrelid = to_regclass('project_milestone')
  ) THEN
    ALTER TABLE "project_milestone"
      ADD CONSTRAINT "project_milestone_project_id_fkey"
      FOREIGN KEY ("project_id")
      REFERENCES "bug_project"("project_id")
      ON DELETE RESTRICT
      ON UPDATE CASCADE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_milestone_owner_id_fkey'
      AND conrelid = to_regclass('project_milestone')
  ) THEN
    ALTER TABLE "project_milestone"
      ADD CONSTRAINT "project_milestone_owner_id_fkey"
      FOREIGN KEY ("owner_id")
      REFERENCES "sys_user"("user_id")
      ON DELETE SET NULL
      ON UPDATE CASCADE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bug_project_version_iteration_id_fkey'
      AND conrelid = to_regclass('bug_project_version')
  ) THEN
    ALTER TABLE "bug_project_version"
      ADD CONSTRAINT "bug_project_version_iteration_id_fkey"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bug_project_version_milestone_id_fkey'
      AND conrelid = to_regclass('bug_project_version')
  ) THEN
    ALTER TABLE "bug_project_version"
      ADD CONSTRAINT "bug_project_version_milestone_id_fkey"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_project_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_project_id_fkey"
      FOREIGN KEY ("project_id")
      REFERENCES "bug_project"("project_id")
      ON DELETE RESTRICT
      ON UPDATE CASCADE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_module_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_module_id_fkey"
      FOREIGN KEY ("module_id")
      REFERENCES "bug_project_module"("module_id")
      ON DELETE SET NULL
      ON UPDATE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_owner_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_owner_id_fkey"
      FOREIGN KEY ("owner_id")
      REFERENCES "sys_user"("user_id")
      ON DELETE SET NULL
      ON UPDATE CASCADE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_developer_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_developer_id_fkey"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_tester_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_tester_id_fkey"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_iteration_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_iteration_id_fkey"
      FOREIGN KEY ("iteration_id")
      REFERENCES "project_iteration"("iteration_id")
      ON DELETE SET NULL
      ON UPDATE CASCADE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_milestone_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_milestone_id_fkey"
      FOREIGN KEY ("milestone_id")
      REFERENCES "project_milestone"("milestone_id")
      ON DELETE SET NULL
      ON UPDATE CASCADE;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_requirement_version_id_fkey'
      AND conrelid = to_regclass('project_requirement')
  ) THEN
    ALTER TABLE "project_requirement"
      ADD CONSTRAINT "project_requirement_version_id_fkey"
      FOREIGN KEY ("version_id")
      REFERENCES "bug_project_version"("version_id")
      ON DELETE SET NULL
      ON UPDATE CASCADE;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bug_ticket_requirement_id_fkey'
      AND conrelid = to_regclass('bug_ticket')
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bug_ticket_iteration_id_fkey'
      AND conrelid = to_regclass('bug_ticket')
  ) THEN
    ALTER TABLE "bug_ticket"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'bug_ticket_milestone_id_fkey'
      AND conrelid = to_regclass('bug_ticket')
  ) THEN
    ALTER TABLE "bug_ticket"
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_activity_project_id_fkey'
      AND conrelid = to_regclass('project_activity')
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'project_activity_operator_id_fkey'
      AND conrelid = to_regclass('project_activity')
  ) THEN
    ALTER TABLE "project_activity"
      ADD CONSTRAINT "project_activity_operator_id_fkey"
