...
export class CreateRequirementDto {
  ...
  @ApiPropertyOptional({ description: '需求来源' }) @IsOptional() @IsString() source?: string
  @ApiPropertyOptional({ description: '优先级' }) @IsOptional() @IsString() priority?: string
  @ApiPropertyOptional({ description: '业务价值分' }) @IsOptional() @Type(() => Number) @IsInt() @Min(0) @Max(100) valueScore?: number
  @ApiPropertyOptional({ description: '实现难度分' }) @IsOptional() @Type(() => Number) @IsInt() @Min(0) @Max(100) difficultyScore?: number
  ...
  @ApiPropertyOptional({ description: '里程碑ID' }) @IsOptional() @IsString() milestoneId?: string
  @ApiPropertyOptional({ description: '目标版本ID' }) @IsOptional() @IsString() versionId?: string
  @ApiPropertyOptional({ description: '计划开始时间' }) @IsOptional() @IsString() plannedStartTime?: string
  @ApiPropertyOptional({ description: '计划完成时间' }) @IsOptional() @IsString() plannedEndTime?: string
  @ApiPropertyOptional({ description: '需求描述' }) @IsOptional() @IsString() description?: string
  @ApiPropertyOptional({ description: '验收标准' }) @IsOptional() @IsString() acceptanceCriteria?: string
...
export class CreateMilestoneDto {
  ...
  @ApiPropertyOptional({ description: '完成条件' }) @IsOptional() @IsString() completionCriteria?: string
  @ApiPropertyOptional({ description: '备注' }) @IsOptional() @IsString() remark?: string
}
...
export class UpdateProjectProgressDto {
  ...
  @ApiPropertyOptional({ description: '计划开始时间' }) @IsOptional() @IsString() plannedStartTime?: string
  @ApiPropertyOptional({ description: '计划完成时间' }) @IsOptional() @IsString() plannedEndTime?: string
  @ApiPropertyOptional({ description: '实际开始时间' }) @IsOptional() @IsString() actualStartTime?: string
  @ApiPropertyOptional({ description: '实际完成时间' }) @IsOptional() @IsString() actualEndTime?: string
  @ApiPropertyOptional({ description: '项目进度 0-100' }) @IsOptional() @Type(() => Number) @IsInt() @Min(0) @Max(100) progress?: number
