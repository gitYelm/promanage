...
export class ExecutiveDashboardService {
  constructor(private readonly prisma: PrismaService, private readonly query: ProjectDashboardQueryService) {}
  ...
  async summary(input: ProjectDashboardQueryDto, user: RequestUserLike) {
    ...
    const now = new Date()
    const [totalProjects, riskProjects, delayedProjects, upcomingMilestones, counts] = await Promise.all([
      this.prisma.bugProject.count({ where: await this.query.projectWhere(user, input) }),
      this.prisma.bugProject.count({ where: { AND: [await this.query.projectWhere(user, input), { riskLevel: { in: [PM_RISK_LEVEL.MEDIUM, PM_RISK_LEVEL.HIGH, PM_RISK_LEVEL.DELAYED] } }] } }),
      this.prisma.bugProject.count({ where: { AND: [await this.query.projectWhere(user, input), { plannedEndTime: { lt: now }, projectStage: { notIn: ['released', 'maintenance', 'archived'] } }] } }),
      this.prisma.projectMilestone.count({ where: { projectId: { in: projectIds }, delFlag: '0', targetDate: { gte: now, lte: this.daysLater(7) } } }),
      this.query.workCounts(user, projectIds, input),
    ])
    return { totalProjects, normalProjects: Math.max(totalProjects - riskProjects - delayedProjects, 0), riskProjects, delayedProjects, upcomingMilestones, ...counts, lastUpdatedAt: new Date() }
  }

  async projects(input: ProjectDashboardQueryDto, user: RequestUserLike) {
    const projects = await this.prisma.bugProject.findMany({ where: await this.query.projectWhere(user, input), include: { owner: true }, orderBy: [{ riskLevel: 'desc' }, { plannedEndTime: 'asc' }], take: Number(input.pageSize ?? 50) })
    return Promise.all(projects.map(async (project) => {
      const baseBugWhere = await this.query.scopedBugWhere(user, [project.projectId], undefined, input)
      const [totalReq, doneReq, totalBug, closedBug, blockerBugs, delayedMilestones] = await Promise.all([
        this.prisma.projectRequirement.count({ where: { projectId: project.projectId, delFlag: '0' } }),
        this.prisma.projectRequirement.count({ where: { projectId: project.projectId, delFlag: '0', status: { in: this.query.requirementDoneStatuses() } } }),
  ...
  async risks(input: ProjectDashboardQueryDto, user: RequestUserLike) {
    ...
    const [blockingBugs, delayedMilestones, pendingRequirements] = await Promise.all([
      this.prisma.bugTicket.findMany({ where: { AND: [await this.query.scopedBugWhere(user, projectIds), { severity: { in: ['blocker', 'critical'] }, status: { not: 'closed' } }] }, include: { project: true, assignee: true }, take: 20 }),
      this.prisma.projectMilestone.findMany({ where: { projectId: { in: projectIds }, delFlag: '0', status: { notIn: ['achieved', 'cancelled'] }, targetDate: { lt: new Date() } }, include: { project: true, owner: true }, take: 20 }),
      this.prisma.projectRequirement.findMany({ where: this.query.requirementWhere(projectIds, PM_DASHBOARD_GROUPS.requirementPending, input), include: { project: true, owner: true }, take: 20 }),
    ])
  ...
  async upcoming(input: ProjectDashboardQueryDto, user: RequestUserLike) {
    const projectIds = await this.query.visibleProjectIds(user, input)
    return this.prisma.projectMilestone.findMany({ where: { projectId: { in: projectIds }, delFlag: '0', targetDate: { gte: new Date(), lte: this.daysLater(30) } }, include: { project: true, owner: true }, orderBy: { targetDate: 'asc' }, take: 30 })
  }
